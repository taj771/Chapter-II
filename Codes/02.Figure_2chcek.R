...
df_revneue <- df_all%>%
  select(Site,year,reve_val_mm_wheat,reve_val_mm_canola,reve_val_mm_w)%>%
  pivot_longer(cols = c(`reve_val_mm_wheat`, `reve_val_mm_canola`,`reve_val_mm_w`), names_to = "variable", values_to = "value")
  
df_revneue$variable <- factor(df_revneue$variable, levels = c("reve_val_mm_wheat", "reve_val_mm_canola","reve_val_mm_w"))

p <- ggplot(df_revneue, aes(x = factor(year), y = value,  fill = interaction(variable))) +
  ...
  scale_y_continuous(
                     limits = c(0, 1.5), breaks = seq(-2, 2, by = 0.5)) + # Customize y-axis scale
...
ggsave("/Users/tharakajayalath/Library/CloudStorage/OneDrive-UniversityofSaskatchewan/Chapter II-IrrigationValue/results/images/AverageValue_Revenue.png", plot = p, width = 10, height = 7, dpi = 100)
...
p <- ggplot(df_profit, aes(x = factor(year), y = value,  fill = interaction(variable))) +
  ...
  theme(panel.grid.major = element_blank(),  # Remove major grid lines
        ...
        axis.line = element_line(color = "black")) +
  scale_y_continuous(
                     limits = c(-2, 1.5), breaks = seq(-2, 2, by = 0.5)) + # Customize y-axis scale
  guides(fill = guide_legend(title = NULL)) +  # Remove legend title
  theme(
    legend.position = "bottom",  # Move the legend to the bottom
    axis.text.x = element_text(size = 12),  # Adjust x-axis text size
...
ggsave("/Users/tharakajayalath/Library/CloudStorage/OneDrive-UniversityofSaskatchewan/Chapter II-IrrigationValue/results/images/AverageValue_Profit.png", plot = p, width = 10, height = 7, dpi = 100)
