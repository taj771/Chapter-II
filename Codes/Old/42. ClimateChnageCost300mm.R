...
merged_wheat_20redP <- map_dfr(years, ~read_csv(paste0("AquaCropOPSyData/WheatCMIP126/RedPrcp/300mm/WheatCMIP126_20redPrcp", .x, ".csv")) %>%
                                 mutate(year = .x) %>%
...
merged_wheat_40redP <- map_dfr(years, ~read_csv(paste0("AquaCropOPSyData/WheatCMIP126/RedPrcp/300mm/WheatCMIP126_40redPrcp", .x, ".csv")) %>%
                                 mutate(year = .x) %>%
...
merged_wheat_60redP <- map_dfr(years, ~read_csv(paste0("AquaCropOPSyData/WheatCMIP126/RedPrcp/300mm/WheatCMIP126_60redPrcp", .x, ".csv")) %>%
                                 mutate(year = .x) %>%
                                 rename(yld_60red = Yield_tonne_per_ha,
                                        irq_60red = Total_Irrigation_mm) %>%
                                 select(year, Site_ID, yld_60red, irq_60red))

merged_wheat_80redP <- map_dfr(years, ~read_csv(paste0("AquaCropOPSyData/WheatCMIP126/RedPrcp/300mm/WheatCMIP126_80redPrcp", .x, ".csv")) %>%
                                 mutate(year = .x) %>%
...
p <- ggplot(df_final_long, aes(x = year, y = returns, color = scenario, group = scenario)) +
  ...
  scale_y_continuous(breaks = c(0,5,10,15,20,25,30,35,40)) +  # Custom breaks
  theme_minimal() +
...
ggsave("/Users/tharakajayalath/Library/CloudStorage/OneDrive-UniversityofSaskatchewan/Chapter II-IrrigationValue/results/images/ClimateChnageCost300mm.png", plot = p, width = 10, height = 7, dpi = 300)
